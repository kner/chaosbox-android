...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    ...
    echo "Installiert SSHCopy-release.apk. Vorher bash ./sign.sh ausführen."
    exit 0
fi
if (( $# > 1 )); then
...
apk="$PWD/SSHCopy-release.apk"
[[ -f "$apk" ]] || { echo "Fehler: APK fehlt. Zuerst bash ./sign.sh ausführen." >&2; exit 1; }
...
echo "Release-APK auf $serial installiert."
